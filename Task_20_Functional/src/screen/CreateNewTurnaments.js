...
const CreateNewTurnaments = () => {
  const [selectedItems, setSelectedItems] = useState([]);
  const [selectedSports, setSelectedSports] = useState('Select Sport');
  const [modalVisible, setModalVisible] = useState(false);
  const [isFormatVisible, setIsFormatVisible] = useState(false);
  const [teamSelect, setTeamSelect] = useState(true);
  const [individualSelect, setIndividualSelect] = useState(false);
  ...
  const [SelectFormatData] = useState([
    {
  ...
  const selectFormat = (selectedItem) => {
    const index = selectedItems.findIndex(
      (item) => item.LeagueName === selectedItem.LeagueName
    );
...
export default CreateNewTurnaments;
